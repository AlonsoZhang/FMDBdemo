...
-(void)handleUpdate:(UIButton *)sender {
    ...
    //3.
    BOOL result = [db executeUpdate:@"update 't_student' set ID = ? where name = ?" withArgumentsInArray:@[@114,@"x3"]];
    //4.
...
-(void)handleQuery:(UIButton *)sender {
    ...
    //3.
    FMResultSet *result = [db executeQuery:@"select * from 't_student' where ID = ?" withArgumentsInArray:@[@114]];
    //4
...
- (void)handleMutilLine:(UIButton *)sender {
    ...
    dispatch_group_async(group, queue, ^{
        BOOL result = [db executeUpdate:@"insert into text3(ID,name,age) values(:ID,:name,:age)"
                withParameterDictionary:@{@"ID":@11,@"name":@"11",@"age":@11}];
        if (result) {
    ...
    dispatch_group_notify(group, queue, ^{
        NSLog(@"done");
        NSLog(@"current thread = %@",[NSThread currentThread]);
        FMResultSet *result = [db executeQuery:@"select * from text3 where ID = ?",@(10)];
        if (result) {
            NSLog(@"query 10 success %@",[result stringForColumn:@"name"]);
        }
        FMResultSet *result2 = [db executeQuery:@"select * from text3 where ID = ?",@(11)];
        if (result2) {
            NSLog(@"query 11 success %@",[result2 stringForColumn:@"name"]);
        }
        FMResultSet *result3 = [db executeQuery:@"select * from text3 where ID = ?",@(12)];
        if (result3) {
            NSLog(@"query 12 success %@",[result3 stringForColumn:@"name"]);
        }
